...
public interface TradeIcon {
    ...
    /**
     * @param layout The editing trade layout.
     * @param player The editing player.
     * @return An item to represent this trade icon in the layout editor.
     */
    @NotNull ItemStack getEditorIcon(@NotNull TradeLayout layout, @NotNull Player player);
    ...
    default void log(Trade trade, String message) {
        if (trade.isInitiationServer()) getTradeLog().log(trade.getPlayers()[0], trade.getPlayers()[1], message);
        else {
            //exception -> proxy trade -> handle money only on one server -> switch players
            getTradeLog().log(trade.getPlayers()[1], trade.getPlayers()[0], message);
        }
    }
}
